...
export async function GET(request: Request) {
  ...
  if (!code) {
    ...
    return NextResponse.redirect(
      `${env.NEXT_PUBLIC_ORIGIN}/api/supabase/auth/callback/link-malformed/`
    );
  }
  ...
  let userSessionData;
  {
    ...
    if (response.error) {
      ...
      // return the user to an error page with instructions
      return NextResponse.redirect(
        `${env.NEXT_PUBLIC_ORIGIN}/api/supabase/auth/callback/link-did-not-work/`
      );
    }
  ...
  let userMetaData;
  {
    ...
    if (result.success === false) {
      ...
      // return the user to an error page with instructions
      return NextResponse.redirect(
        `${env.NEXT_PUBLIC_ORIGIN}/api/supabase/auth/callback/couldnt-create-user-metadata`
      );
    }
